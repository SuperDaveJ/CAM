...
v 2.2 last update July 25, 2005
*/
...
FlashSound.requiredVersion = "4,0,0,0"; // default to 4, change with intiateAutoInstall method
function fs_initiateAutoInstall(req_vers){
	FlashSound.requiredVersion = req_vers; // assign higher version to requiredVersion property
	
	// for plug-in version detection only
	var autoInstall = false;
	if(FlashSound.Plugin){
		var req_vers_parts = req_vers.split(",");
		var req_major_vers = req_vers_parts[0] + "." + req_vers_parts[1];
		var req_r_vers = req_vers_parts[2] + "." + req_vers_parts[3];
	
		var installed_major_vers = fs_getPlugInVers();
		var installed_r_vers = fs_getPlugInReleaseVers();
	
		if(req_major_vers > installed_major_vers){
			var autoInstall = true;
		} else if (req_major_vers == installed_major_vers && req_r_vers > installed_major_vers){
			var autoInstall = true;
		} else {
			var autoInstall = false;
		}
...
FlashSound.initiateAutoInstall = fs_initiateAutoInstall;
...
function fs_setAttributeDefaultValues(){
	...
	// detect if server uses HTTPS/SSL and set CODEBASE protocol accordingly to prevent IE from
	// displaying message about mixed secure/unsecure content to user
	var cb_protocol = (window.location.href.indexOf("https://") > -1)? "https" : "http";
	this.attributes["CODEBASE"]= cb_protocol + "://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=" + FlashSound.requiredVersion;
...
function fs_getAttributeValue(attributeName){
	if(FlashSound.configuredForInteraction() && this.attributes != null && this.attributes[attributeName] != null){
